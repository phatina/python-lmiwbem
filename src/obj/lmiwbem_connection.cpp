...
WBEMConnection::ScopedConnection::ScopedConnection(WBEMConnection *conn)
    : m_conn(conn)
    , m_conn_orig_state(m_conn->m_rep->m_client.isConnected())
{
    if (m_conn_orig_state) {
        // We are already connected, nothing to do here.
        return;
    } else if (m_conn->m_rep->m_connect_locally) {
        m_conn->m_rep->m_client.connectLocally();
        return;
    } else if (m_conn->m_rep->m_url.empty()) {
        throw_ValueError("WBEMConnection constructed without url parameter");
    }

    try {
        m_conn->m_rep->m_client.connect(
            m_conn->m_rep->m_url,
            m_conn->m_rep->m_username,
            m_conn->m_rep->m_password,
            m_conn->m_rep->m_cert_file,
            m_conn->m_rep->m_key_file,
            Config::defaultTrustStore());
    } catch (...) {
        std::stringstream ss;
        if (Config::isVerbose()) {
            bool connect_locally = m_conn->m_rep->m_connect_locally;
            if (connect_locally)
                ss << "connect_locally(";
            else
                ss << "connect(";
            ...
            if (Config::isVerboseMore() && !connect_locally) {
                ss << "url='" << m_conn->m_rep->m_client.getURLInfo().asString() << '\'';
            }
            ss << ')';
        }
...
WBEMConnection::WBEMConnection(
    ...
    : m_rep(new WBEMConnectionRep)
{
    m_rep->m_connect_locally = Conv::as<bool>(connect_locally, "connect_locally");

    // We are constructing with local connection flag; disregard other
    // parameters.
    if (m_rep->m_connect_locally)
        return;

    if (!isnone(url))
        m_rep->m_url = StringConv::asString(url, "url");
    ...
    if (!isnone(x509)) {
        ...
        if (!isnone(py_cert_file)) {
            m_rep->m_cert_file = StringConv::asString(
                py_cert["cert_file"], "cert_file");
        }
        if (!isnone(py_key_file)) {
            m_rep->m_key_file  = StringConv::asString(
                py_cert["key_file"], "key_file");
        }
    }

    bool c_verify = Conv::as<bool>(no_verification, "no_verification");
    m_rep->m_client.setVerifyCertificate(c_verify);

    if (!isnone(default_namespace)) {
        m_rep->m_default_namespace = StringConv::asString(
            default_namespace, "default_namespace");
    }

    // Set timeout to 1 minute.
    m_rep->m_client.setTimeout(60000);
}

WBEMConnection::~WBEMConnection()
{
    m_rep->m_client.disconnect();
}
...
{
    std::stringstream ss;
    ss << "WBEMConnection(url=u'" << m_rep->m_url << "', ...)";
    return ss.str();
}
    ...
    const bp::object &no_verification)
{
    ...
    String c_url(m_rep->m_url);
    String c_cert_file(m_rep->m_cert_file);
    String c_key_file(m_rep->m_key_file);
    ...
    if (!isnone(username))
        m_rep->m_username = StringConv::asString(username, "username");
    if (!isnone(password))
        m_rep->m_password = StringConv::asString(password, "password");
    if (!isnone(cert_file))
    ...
    if (!isnone(no_verification)) {
        ...
        m_rep->m_client.setVerifyCertificate(!c_no_verify);
    }

    try {
        m_rep->m_client.connect(
            c_url,
            m_rep->m_username,
            m_rep->m_password,
            c_cert_file,
            c_key_file,
            Config::defaultTrustStore());
        m_rep->m_connect_locally = false;
    } catch (...) {
        std::stringstream ss;
        if (Config::isVerbose()) {
            ss << "connect(";
            if (Config::isVerboseMore())
                ss << "url='" << m_rep->m_client.getURLInfo().asString() << '\'';
            ss << ')';
        }
        handle_all_exceptions(ss);
    }
...
{
    m_rep->m_client.connectLocally();
    m_rep->m_connect_locally = true;
} catch (...) {
    std::stringstream ss;
    if (Config::isVerbose())
        ss << "connect_locally()";
...
void WBEMConnection::setRequestAcceptLanguages(const bp::object &languages)
{
    ...
    m_rep->m_client.setRequestAcceptLanguages(peg_al_list);
}

bp::object WBEMConnection::getDefaultNamespace() const
{
    return StringConv::asPyUnicode(m_rep->m_default_namespace);
}

void WBEMConnection::setDefaultNamespace(const bp::object &ns)
{
    m_rep->m_default_namespace = StringConv::asString(ns, "default_namespace");
}

bp::object WBEMConnection::getCredentials() const
{
    return bp::make_tuple(
        StringConv::asPyUnicode(m_rep->m_username),
        StringConv::asPyUnicode(m_rep->m_password));
}
...
void WBEMConnection::setCredentials(const bp::object &creds)
{
    ...
    m_rep->m_username = StringConv::asString(py_creds_tpl[0], "username");
    m_rep->m_password = StringConv::asString(py_creds_tpl[1], "password");
}
    ...
    const bp::object &ns) try
{
    ...
    peg_new_inst_name = m_rep->m_client.createInstance(
    ...
    peg_new_inst_name.setHost(m_rep->m_client.hostname());
    ...
    const bp::object &property_list) try
{
    ...
    ScopedTransactionBegin();
    peg_instances = m_rep->m_client.enumerateInstances(
        peg_ns,
        peg_name,
        deep_inheritance,
        local_only,
    ...
    return ListConv::asPyCIMInstanceList(
        peg_instances, c_ns, m_rep->m_client.hostname());
} catch (...) {
    std::stringstream ss;
    if (Config::isVerbose()) {
    ...
    const bp::object &ns) try
{
    ...
    ScopedTransactionBegin();
    peg_instance_names = m_rep->m_client.enumerateInstanceNames(
        peg_ns,
        peg_name);
    ScopedTransactionEnd();

    return ListConv::asPyCIMInstanceNameList(
        peg_instance_names, c_ns, m_rep->m_client.hostname());
} catch (...) {
    std::stringstream ss;
    if (Config::isVerbose()) {
    ...
    const bp::dict  &kwds) try
{
    ...
    ScopedTransactionBegin();
    peg_rval = m_rep->m_client.invokeMethod(
        peg_ns,
        peg_path,
        peg_name,
        peg_in_params,
    ...
    const bp::object &property_list) try
{
    ...
    ScopedTransactionBegin();
    peg_instance = m_rep->m_client.getInstance(
        peg_ns,
        peg_object_path,
        local_only,
        include_qualifiers,
    ...
    const bool include_class_origin) try
{
    ...
    ScopedTransactionBegin();
    peg_classes = m_rep->m_client.enumerateClasses(
        peg_ns,
        peg_classname,
        deep_inheritance,
        local_only,
    ...
    const bool deep_inheritance) try
{
    ...
    ScopedTransactionBegin();
    peg_classnames = m_rep->m_client.enumerateClassNames(
        peg_ns,
        peg_classname,
        deep_inheritance);
    ScopedTransactionEnd();
    ...
    const bp::object &ns) try
{
    ...
    ScopedTransactionBegin();
    peg_instances = m_rep->m_client.execQuery(
        peg_ns,
        peg_query_lang,
        peg_query);
    ScopedTransactionEnd();
    ...
    const bp::object &property_list) try
{
    ...
    ScopedTransactionBegin()
    peg_class = m_rep->m_client.getClass(
        peg_ns,
        peg_name,
        local_only,
        include_qualifiers,
    ...
    const bp::object property_list) try
{
    ...
    ScopedTransactionBegin();
    peg_associators = m_rep->m_client.associators(
        peg_ns,
        peg_path,
        peg_assoc_class,
        peg_result_class,
    ...
    return ListConv::asPyCIMInstanceList(
        peg_associators, c_ns, m_rep->m_client.hostname());
} catch (...) {
    std::stringstream ss;
    if (Config::isVerbose()) {
        ss << "Associators(";
    ...
    const bp::object &result_role) try
{
    ...
    ScopedTransactionBegin();
    peg_associator_names = m_rep->m_client.associatorNames(
        peg_ns,
        peg_path,
        peg_assoc_class,
        peg_result_class,
    ...
    return ListConv::asPyCIMInstanceNameList(
        peg_associator_names, c_ns, m_rep->m_client.hostname());
} catch (...) {
    std::stringstream ss;
    if (Config::isVerbose()) {
        ss << "AssociatorNames(";
    ...
    const bp::object &property_list) try
{
    ...
    ScopedTransactionBegin();
    peg_references = m_rep->m_client.references(
        peg_ns,
        peg_path,
        peg_result_class,
        c_role,
    ...
    return ListConv::asPyCIMInstanceList(
        peg_references, c_ns, m_rep->m_client.hostname());
} catch (...) {
    std::stringstream ss;
    if (Config::isVerbose()) {
        ss << "References(";
    ...
    const bp::object &role) try
{
    ...
    ScopedTransactionBegin();
    peg_reference_names = m_rep->m_client.referenceNames(
        peg_ns,
        peg_path,
        peg_result_class,
        c_role);
    ...
    return ListConv::asPyCIMInstanceNameList(
        peg_reference_names, c_ns, m_rep->m_client.hostname());
} catch (...) {
    std::stringstream ss;
    if (Config::isVerbose()) {
        ss << "ReferenceNames(";
